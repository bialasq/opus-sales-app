...
/** Rola arkusza w mapowaniu kolumn (cache per odcisk struktury pliku). */
export type SheetRole = "sales" | "visits" | "payments" | "ignore";

/** Zapisane mapowanie kolumn dla danego fingerprintu. */
export interface StoredMapping {
  sheetRoles: Record<string, SheetRole>;
  /** columns[sheetName][poleDomenowe] = nazwa kolumny w pliku lub null */
  columns: Record<string, Record<string, string | null>>;
  createdAt: string;
  source: "agent" | "manual";
}

/** Organization.settings (JSON) — rozszerzalne o kolejne pola. */
