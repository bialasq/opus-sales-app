...
import { computeFingerprint } from "./fileStructureFingerprint";
...
class ExcelService {
  async readAndValidateBuffer(
    buffer: Buffer,
    filename: string,
    organizationId: string
  ): Promise<ValidatedExcelWorkbook> {
    const raw = parseRawWorkbookBuffer(buffer);
    const validated = validateExcelWorkbook(raw, filename);

    if (
      validated.sales.length ||
      validated.visits.length ||
      validated.payments.length
    ) {
      return validated;
    }

    const fingerprint = computeFingerprint(raw);
    let mapping = await getMapping(organizationId, fingerprint);

    if (!mapping) {
      try {
        mapping = await mapWorkbookStructure(raw, organizationId);
      } catch (err) {
        const detail = err instanceof Error ? err.message : String(err);
        throw new ValidationError(`Nie rozpoznano formatu pliku: ${detail}`);
      }
      await saveMapping(organizationId, fingerprint, mapping);
    }
  ...
  async readFile(
    ...
    try {
      ...
      return await this.readAndValidateBuffer(buffer, filename, organizationId);
    } catch (error: unknown) {
