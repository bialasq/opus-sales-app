...
/** Kontekst danych sprzedażowych powiązany z plikiem w uploads/ */
...
async function toolPredictFutureSales(
  ...
  return predictFutureSales(ctx.filename, { productName, horizonDays });
}
...
async function toolCompareWithPreviousPeriod(
  ...
  if (!candidates.length) {
    return {
      found: false,
      message: "Brak wcześniejszego pliku Excel w uploads/ — wgraj drugi plik aby porównać okresy",
    };
  }
